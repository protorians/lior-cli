...
func (v *Validator) ValidateModule(name string) (*Result, error) {
	...
	// domain format warning (spec §5.10: expected mod.sentients.<name>)
	addLevel(res, "manifest.json", "domain", isSentientDomain(manifest.Domain),
		"domain in mod.sentients.<name> format", LevelWarning)
...
// isSentientDomain reports whether the domain matches the expected
// mod.sentients.<name> format generated by `sentients create module` (spec §5.10).
func isSentientDomain(domain string) bool {
	if !strings.HasPrefix(domain, "mod.sentients.") {
		return false
	}
	suffix := strings.TrimPrefix(domain, "mod.sentients.")
